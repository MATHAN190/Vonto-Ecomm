      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {categories.map((cat) => (
          <Link key={cat.id} to={`/category/${cat.name.toLowerCase()}`}>
            <CatCategory name={cat.name} img={cat.img} />
          </Link>
        ))}
      </div>
    </div>
...
// ---- Cat_Category inside this file ----
function CatCategory({ name, img }) {
  return (
    <div className="bg-white p-4 rounded-xl shadow hover:shadow-lg transition flex flex-col justify-center items-center">
      <img
